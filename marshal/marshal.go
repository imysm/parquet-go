...
func MarshalFlat(rows [][]any, schemaHandler *schema.SchemaHandler, columnPaths []string) (result *map[string]*layout.Table, err error) {
	defer func() {
		if r := recover(); r != nil {
			switch x := r.(type) {
			case string:
			...
			case error:
				err = x
			default:
				err = errors.New("unknown error in MarshalFlat")
			}
		}
	}()
